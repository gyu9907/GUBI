...
$(document).ready(function () {
    ...
    const func_email = (e) => {
        ...
        if (email == "") {
            ...
            if (!bool) {
                ...
                $("div#emailerror").html("");
                is_email_ok = true;
                // console.log("is_email_ok3 => ", is_email_ok);
            }
        }
    ...
    */
    ...
    console.log("is_email_ok => ",is_email_ok);
    
    // 이메일 체크 버튼 숨겨줌
    $("button#emailcheck").hide();

    // 이메일 변경을 누르면 
    $("button#btn_email_change").click(function (e) { 
        
        $(e.target).hide(); // 자기 버튼 숨기고
        $("button#emailcheck").show();
        $("input#email").removeClass("disabled"); // disabled 제거
        is_email_ok = false; // 이메일 체크를 바꿈
        b_email_auth_click = false; // 이메일 확인했다는 체크버튼도 바꿈
        $("input#email").focus();
        // console.log("is_email_ok2 => ",is_email_ok);

    });//end of $("button#btn_email_change").click(function (e) { }...
