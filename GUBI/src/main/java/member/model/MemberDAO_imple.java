	...
	public List<MemberVO> optionMemberList(Map<String, String> paraMap) throws SQLException {
		...
		try {
			...
			String sql =  " select * "
						...
						+ "     	to_char(registerday, 'yyyy-mm-dd') as registerday, status, idle, point "
						+ "     	from tbl_member "
						+ " 	), "
						+ " 	c as "
						+ " 	( "
						...
						+ " 	d as "
						+ " 	( "
						+ "   		select rank() over(partition by fk_userid order by loginday desc) as rank, "
						+ "			to_char(loginday, 'yyyy-mm-dd') as loginday , "
						+ "			fk_userid "
						+ "    		from tbl_login "
						+ " 	) "
