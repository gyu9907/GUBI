...
public class AdminCollectionListJSON extends AbstractController {
	...
	public void execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if(searchType == null ||
			...
			"collectionno".equals(searchType))) {
			searchType = "";
		}
		...
		/*** 페이지 바 만들기 시작 ***/
		int totalCollectionCnt = coldao.totalCollectionCntForAdmin(paraMap);
		...
		pageBar += "<li class='page-item'><a class='page-link' href='javascript:searchCollection(\""+request.getContextPath()+"\", 1)'>&laquo;</a></li>";

        // 첫번째 페이지 바에서는 다음 버튼을 표시하지 않는다.
		if (pageNo > 1) {
			pageBar += "<li class='page-item'><a class='page-link' href='javascript:searchCollection(\""+request.getContextPath()+"\", "+(pageNo-1)+")'>&lt;</a></li>";
		}

        while(!(loop > blockSize || pageNo > totalPage)) {
        	
        	if(pageNo == Integer.parseInt(currentShowPageNo)) {
	        	pageBar += "<li class='page-item active'><a class='page-link' href='javascript:searchCollection(\""+request.getContextPath()+"\", "+pageNo+")'>"+pageNo+"</a></li>";
        	}
        	else {
	        	pageBar += "<li class='page-item'><a class='page-link' href='javascript:searchCollection(\""+request.getContextPath()+"\", "+pageNo+")'>"+pageNo+"</a></li>";
        	}
        ...
        if(pageNo <= totalPage) {
			pageBar += "<li class='page-item'><a class='page-link' href='javascript:searchCollection(\""+request.getContextPath()+"\", "+pageNo+")'>&gt;</a></li>";
        }
		pageBar += "<li class='page-item'><a class='page-link' href='javascript:searchCollection(\""+request.getContextPath()+"\", "+totalPage+")'>&raquo;</a></li>";
		/*** 페이지 바 만들기 끝 ***/
		
		try {
			List<CollectionVO> collectionList = coldao.getCollectionListForAdmin(paraMap);
