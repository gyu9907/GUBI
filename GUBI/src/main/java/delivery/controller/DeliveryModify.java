...
public class DeliveryModify extends AbstractController {
    ...
    public void execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        if (deliveryno == null || deliveryno.isBlank()) { 
        	...
        	super.setViewPage(request.getContextPath() + "/delivery/deliveryList.gu "); // 배송지 목록 페이지로 이동
        	return;
        }
        ...
        if ("GET".equals(method)) {
            ...
            if (delivery == null) { 
            	super.setRedirect(true);
            	super.setViewPage(request.getContextPath() + "/delivery/deliveryList.gu "); // 수정 페이지로 이동
            } else {
            	 request.setAttribute("delivery", delivery);
             	 super.setRedirect(false);
                 super.setViewPage("/WEB-INF/delivery/deliveryModify.jsp"); // 수정 페이지로 이동
            }
